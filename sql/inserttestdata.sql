...
insert into `case` (open, title, description, location, time, type_id) 
	select TRUE, "case 1", "description 1", "location 1", "2011-12-18 13:17:17", t.id 
	from type t 
	where t.name = "murder";
insert into `case` (open, title, description, location, time, type_id) 
	select TRUE, "case 2", "description 2", "location 2", "2013-12-18 13:17:17", t.id 
	from type t 
	where t.name = "robbery";

insert into poi (name, birthdate) values ( "alex", "2013-12-19");
insert into poi (name, birthdate) values ( "jimmy", "2013-12-20");

insert into is_linked_to (case_id, poi_id, time, end_time)
	select c.id, p.id, "2011-12-18 13:17:17", "2011-12-21 13:17:17"
	from `case` c, poi p, `type` t
	where c.title = "case 1" and p.name = "alex";
insert into is_linked_to (case_id, poi_id, time) 
	select c.id, p.id, "2013-12-18 13:17:17"
	from `case` c, poi p, `type` t
	where c.title = "case 2" and p.name = "alex";
